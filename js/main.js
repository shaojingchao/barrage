...
(function(win, $) {
  ...
  function _itemFn(item) {
    ...
    function _call(_pos) {
      ...
      var _transX = '-' + (_size.width + 100 + $item.innerWidth()) + 'px';
      var _css = '{' +
        '"top":"' + _pos.index * 45 + 'px' + '",' +
        '"transform":' + '"translate' + (Modernizr.csstransforms3d ? '3d(' + _transX + ',0,0)' : '(' + _transX + ')') + '",' +
        '"transition":' + '"transform ' + (_time + 's') + ' linear"' +
      '}';
      $item.css($.parseJSON(PrefixFree.prefixCSS(_css)));
      setTimeout(function() {
        $item.remove();
